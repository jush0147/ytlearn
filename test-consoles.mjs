...
async function testClient(name, version) {
  ...
  try {
    ...
    if (data.playabilityStatus) console.log(`${name} status:`, data.playabilityStatus.status);
  } catch(e) { console.log(`${name} err:`, e.message) }
}

async function run() {
  await testClient('SWITCH', '1.09');
  await testClient('PLAYSTATION_4', '1.00');
  await testClient('TVHTML5_SIMPLY_EMBEDDED_PLAYER', '2.0');
}
run();
